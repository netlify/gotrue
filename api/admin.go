...
func (a *API) adminUserUpdate(w http.ResponseWriter, r *http.Request) error {
	...
	if herr := triggerEventHooks(ctx, a.db, UserModifiedEvent, user, instanceID, config); herr != nil {
		logrus.WithError(herr).WithField("user_id", user.ID).Warn("Failed to trigger user_modified webhook")
	}
...
func (a *API) adminUserDelete(w http.ResponseWriter, r *http.Request) error {
	...
	if herr := triggerEventHooks(ctx, a.db, UserDeletedEvent, user, instanceID, config); herr != nil {
		logrus.WithError(herr).WithField("user_id", user.ID).Warn("Failed to trigger user_deleted webhook")
	}
